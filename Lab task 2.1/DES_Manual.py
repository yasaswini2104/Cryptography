...
def generate_subkeys(key):
    key = permute(key, PC1) # Apply PC1 permutation to key
    C, D = split_blocks(key) # Split into left and right halves
    
    subkeys = []
    for shift in SHIFT_SCHEDULE:
        C = left_shift(C, shift)
        D = left_shift(D, shift)
        subkey = permute(C + D, PC2) # Combine and apply PC2
        subkeys.append(subkey)
    return subkeys

def f_function(R, subkey): #Feistel (F) function
    expanded = permute(R, E) 
    xored = xor_strings(expanded, subkey)
    s_box_output = '' # S-box substitution
    for i in range(8):
...
def des_encrypt(plaintext, key):
    ...
    L, R = split_blocks(block)
    subkeys = generate_subkeys(key)
    for i in range(16): # 16 rounds
        L_new = R
        R_new = xor_strings(L, f_function(R, subkeys[i]))
        L, R = L_new, R_new
    return permute(R + L, IP_INV) # Final permutation
...
def main():
    # Test with both binary and text input
    # Binary test
    ...
    # Text test
    text = "Hello123"  # 8 characters (64 bits)
